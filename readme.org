...
Select an alternative alphas layout, e.g. ~colemak~, ~dvorak~, ~graphite~, ~halmak~, ~qwerty~.  For Colemak Mod-DH, leave as ~default~.  This fork also adds ~graphite~, which is not part of upstream Miryoku.
...
Used to build with a Zephyr/ZMK snippet, e.g. ~studio-rpc-usb-uart~ to enable [[https://zmk.dev/docs/features/studio][ZMK Studio]] over USB.

Multiple space separated snippets can be specified.

For split keyboards, only apply this to the central/left side build; do not apply it to the peripheral/right side.
...
*** Graphite Punctuation and Comma

Fork addition, not part of upstream Miryoku.

~MIRYOKU_ALPHAS_GRAPHITE~ corrects three shifted punctuation keys to match [[https://github.com/rdavison/graphite-layout][canonical Graphite]] (~'~/~-~/~/~ shift to ~_~/~"~/~<~ instead of the HID default ~"~/~_~/~?~), and adds Graphite's comma via a combo on the right pinky's top- and home-row keys (J held with I), since Miryoku's fixed 5+5 grid has no spare column for it. Both are automatic whenever ~alphas~ is ~graphite~. The comma combo additionally requires the board's mapping header to define ~MIRYOKU_GRAPHITE_COMMA_POSITIONS~; currently only ~42/corne~ does.
